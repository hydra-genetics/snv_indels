...
rule freebayes:
    input:
        ...
        samples="alignment/extract/{sample}_{type}_{chr}.bam",
        indexes="alignment/extract/{sample}_{type}_{chr}.bam.bai",
        regions="misc/bed_split/{sample}_{type}_{chr}.bed",
    output:
        vcf=temp("snv_indels/freebayes/{sample}_{type}_{chr}.unfilt.vcf"),
    params:
